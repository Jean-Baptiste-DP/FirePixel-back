...
AppDataSource.initialize()
    .then(async () => {
        ...
        const pixelService = new NewPixelService(
            CursorDynRepository,
            ScreenDynRepository
        );
        ...
        const saveScreenDynRepo = setInterval(() => {
            ScreenDynRepository.saveInDB();
        }, 10 * 60 * 1000); // save Screen every 10 minutes
        ...
        wss.on("connection", function connection(ws) {
            ws.on("message", async function incoming(string: string) {
                ...
                if (data?.req == "connection_screen" && data.type) {
                    ...
                    response = await cursorService.move(
                        { idScreen: 1, idCursor: index },
                        data
                    );
                } else if (data?.req == "chgColor" && data.color != undefined) {
                    const index = WsConnection.searchIndexFromClient(ws);
                    response = await pixelService.create(
                        { idScreen: 1, idCursor: index },
                        data
                    );
                ...
                if (
                    ...
                    for (let i = 0; i < WsConnection.nbPhone; i++) {
                        if (
                            WsConnection.phones[i] &&
                            WsConnection.phones[i].readyState == WebSocket.OPEN
                        ) {
                            WsConnection.phones[i].send(
                    ...
                    response?.req == "connection_screen" &&
                    ...
                    response?.req == "connection_screen" &&
                    response.type == "screen" &&
                    response.res == "Error : Invalid token"
                ) {
                    console.log(
                        " Failed attemp to log screen , Invalid token: ",
                        response.token
                    );
            ...
            ws.on("close", (event) => {
                ...
                if (index != -1) {
                    ...
                    if (WsConnection.screen) {
                        WsConnection.screen.send(
                            JSON.stringify({ req: "disconnexion", id: index })
                        );
                    }
                }
            });
